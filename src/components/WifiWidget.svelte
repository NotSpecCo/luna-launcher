<script lang="ts">
  ...
  let observer;
  onMount(async () => {
    observer = await Settings.wifi.subscribe((val) => (status = val));
  });

  onDestroy(() => Settings.wifi.unsubscribe(observer));

  async function handleChange(val: boolean) {
    if (val) {
      await Settings.wifi.enable();
    } else {
      await Settings.wifi.disable();
    }
    status = val;
  }
</script>
