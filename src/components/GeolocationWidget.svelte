<script lang="ts">
  ...
  let observer;
  onMount(async () => {
    observer = await Settings.geolocation.subscribe((val) => (status = val));
  });

  onDestroy(() => Settings.geolocation.unsubscribe(observer));

  async function handleChange(val: boolean) {
    if (val) {
      await Settings.geolocation.enable();
    } else {
      await Settings.geolocation.disable();
    }
    status = val;
  }
</script>
