<script lang="ts">
  ...
  let observer;
  onMount(async () => {
    observer = await Settings.usbTethering.subscribe((val) => (status = val));
  });

  onDestroy(() => Settings.usbTethering.unsubscribe(observer));

  async function handleChange(val: boolean) {
    if (val) {
      await Settings.usbTethering.enable();
    } else {
      await Settings.usbTethering.disable();
    }
    status = val;
  }
</script>
