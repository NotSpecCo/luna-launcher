<script lang="ts">
  ...
  let observer;
  onMount(async () => {
    observer = await Settings.cellularData.subscribe((val) => (status = val));
  });

  onDestroy(() => Settings.cellularData.unsubscribe(observer));

  async function handleChange(val: boolean) {
    if (val) {
      await Settings.cellularData.enable();
    } else {
      await Settings.cellularData.disable();
    }
    status = val;
  }
</script>
