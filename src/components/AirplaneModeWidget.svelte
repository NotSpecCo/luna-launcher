<script lang="ts">
  ...
  let observer;
  onMount(async () => {
    observer = await Settings.airplaneMode.subscribe((val) => (status = val));
  });

  onDestroy(() => Settings.airplaneMode.unsubscribe(observer));

  async function handleChange(val: boolean) {
    if (val) {
      await Settings.airplaneMode.enable();
    } else {
      await Settings.airplaneMode.disable();
    }
    status = val;
  }
</script>
